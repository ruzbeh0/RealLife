...
{
    public partial class RealLifeHouseholdSpawnSystem : GameSystemBase
    {
        ...
        private RealLifeHouseholdSpawnSystem.TypeHandle __TypeHandle;
        ...
        protected override void OnUpdate()
        {
            ...
            if (householdDemand > 0)
            {
                ...
                jobHandle = new RealLifeHouseholdSpawnSystem.SpawnHouseholdJob()
                {
                ...
                }.Schedule<RealLifeHouseholdSpawnSystem.SpawnHouseholdJob>(JobUtils.CombineDependencies(outJobHandle1, outJobHandle2, jobHandle, outJobHandle3, deps1, deps2, deps3, deps4));
        ...
        [UnityEngine.Scripting.Preserve]
        public RealLifeHouseholdSpawnSystem()
        {
        }
        ...
        private struct SpawnHouseholdJob : IJob
        {
            ...
            public void Execute()
            {
                int max1 = Mathf.RoundToInt(300f / math.clamp(this.m_DemandParameterData.m_HouseholdSpawnSpeedFactor * 3*math.log((float)(1.0 + 1.0 / 1000.0 * (double)this.m_Populations[this.m_City].m_Population)), 0.5f, 20f));
                int num1 = this.m_Random.NextInt(max1);
                int num2 = 0;

                Mod.log.Info($"max1: {max1}");
                Mod.log.Info($"demand: {this.m_Demand}");
                ...
                int num3 = math.max(10, y1);
                int num4 = math.max(10, y2);
                float num5 = (float)num4 / (float)(num4 + num3);

                Mod.log.Info($"num2: {num2}");
                Mod.log.Info($"num5: {num5}");
                for (int index1 = 0; index1 < num2; ++index1)
                {
                    ...
                    Mod.log.Info($"outside connections {this.m_OutsideConnectionEntities.Length}");
