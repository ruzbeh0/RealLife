...
public partial class RealLifeBirthSystem : GameSystemBase
{
    ...
    {
        return 262144 /*0x040000*/ / (RealLifeBirthSystem.kUpdatesPerDay * 16 /*0x10*/);
    }
    ...
    {
        uint updateFrame = SimulationUtils.GetUpdateFrame(this.m_SimulationSystem.frameIndex, RealLifeBirthSystem.kUpdatesPerDay, 16 /*0x10*/);
        ...
        RealLifeBirthSystem.CheckBirthJob jobData1 = new RealLifeBirthSystem.CheckBirthJob()
        {
            ...
            m_MemberType = InternalCompilerInterface.GetComponentTypeHandle<HouseholdMember>(ref this.__TypeHandle.__Game_Citizens_HouseholdMember_RO_ComponentTypeHandle, ref this.CheckedStateRef),
            ...
            for (int index1 = 0; index1 < nativeArray1.Length; ++index1)
            {
                ...
                if (citizen1.GetAge() == CitizenAge.Adult && (citizen1.m_State & (CitizenFlags.Male | CitizenFlags.Tourist | CitizenFlags.Commuter)) == CitizenFlags.None)
                {
                    ...
                    if (!(property == Entity.Null))
                    {
                        ...
                        int householdSize = householdCitizen.Length;
                        if(householdSize > averageHouseholdSize)
                        {
                            baseBirthRate *= math.pow((float)averageHouseholdSize / (float)householdSize,2);

                            //Mod.log.Info($"Household size {householdSize} is larger than average household size {averageHouseholdSize}, reducing birth rate to {baseBirthRate}");
                        }
